"""Step 03 — TimeSformer motion feature extraction (fixed 8 clips per sample).

Motion features are independent of variable K_temporal; every sample gets a
[8, 768] motion tensor plus [8] normalized centers.

Usage (via sbatch on ASL):
    sbatch slurm/build_features.sbatch scripts/03_extract_motion.py \\
        --split train --allow-full
    sbatch slurm/build_features.sbatch scripts/03_extract_motion.py \\
        --split dev --allow-full

Smoke:
    python scripts/03_extract_motion.py --split dev --limit 3
"""
...
from pgat_length.features.timesformer_extractor import (  # noqa: E402
    TimesformerMotionExtractor,
)
...
def _write_bank_metadata(
    ...
    if path.exists():
        ...
        for key in ("bank", "model_name", "config_fingerprint", "plan_fingerprint"):
            if existing.get(key) != payload[key]:
                raise RuntimeError(f"motion bank metadata mismatch on {key}: {path}")
        return
...
def main() -> None:
    ...
    config_fingerprint = fingerprint_from_sections(
        data_cfg, features_cfg, MOTION_SECTIONS
    )
    combined = {
        "data": data_cfg,
        "features": {k: features_cfg[k] for k in MOTION_SECTIONS if k in features_cfg},
    }
    ...
    output_root = (
        ...
        else feature_root / ("motion" if "smoke" not in plan_root.name else "motion_smoke")
    )
    ...
    motion_cfg = features_cfg.get("motion", {})
    model_name = str(motion_cfg.get("model_name", "facebook/timesformer-base-finetuned-k400"))
    feature_dim = int(motion_cfg.get("feature_dim", 768))
    ...
    extractor: TimesformerMotionExtractor | None = None
    try:
        for shard_number, start in enumerate(range(0, len(reader), shard_size)):
            ...
            shard_path = output_root / args.split / f"motion-{shard_number:05d}.safetensors"
            expected_metadata = {
                "format_version": "2",
                "bank": "motion",
                "split": args.split,
            ...
            if shard_path.exists() and not args.overwrite:
                validate_saved_feature_shard(shard_path, "motion", len(records), expected_metadata)
            ...
            if extractor is None:
                ...
                extractor = TimesformerMotionExtractor(
                    model_name,
            ...
            for reader_index in range(start, stop):
                ...
                sample_arrays.append(
                    {
                        "motion_features": result.features,
                        "motion_centers": result.centers,
                    }
                )
                elapsed = max(time.monotonic() - started, 1e-6)
                completed = reader_index + 1
                print(
                    f"[{completed}/{len(reader)}] {record.uid} rate={completed / elapsed:.3f} samples/s",
                    flush=True,
                )
            arrays = {
